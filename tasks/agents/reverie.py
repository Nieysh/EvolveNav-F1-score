...
class REVERIEAgent(MP3DAgent):
    ...
    def get_spatial_relation_prompt(self, cand_num, QA_landmark, query_cand_id):
        ...
        # Option
        prompt += 'Following is the Option, which includes 6 possible spatial relations. First, check whether the landmarks are at the similar level height to you (upon or under), and if yes, check the horizontal position relation between you and the landmarks (front, rear, right, left).\n'
    ...
    #     # Observation
    #     if self.args.cot_v5:
    #         prompt += 'Following is the Candidate, which contains several directions you can go to at the current position.\n'
    #     else:
    #         prompt += 'Following is the Candidate, which contains several directions you can go to at the current position, candidate (0) is stop.\n'
    ...
    #         elif self.args.cot_v5:
    #             prompt += 'Generate the navigational reasoning and decide the action.\n'
    #             prompt += '- I should go to an observation with {}'.format(land_token) + ' to the {}'.format(dir_token) + ' of me.\n'
    #             prompt += '- Action Decision: {}'.format(cls_token) + '.'
    ...
    def get_navigation_once_forward_cot_navigation_prompt(self, instruction, hist_num, cand_num, cand_landmarks, nav_vpids, cand_masks, cls_token, land_token=None, dir_token=None):
        ...
        prompt += '### Candidate: {}\n'.format(obs_text)
        # Output Hint
        #prompt += 'Explore the scene to find out the targeted room and object. '
