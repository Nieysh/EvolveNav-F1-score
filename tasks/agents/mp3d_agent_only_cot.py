...
class MP3DAgent(BaseAgent):
    ...
    def prepare_cot(self, obs, feedback, data_type, nav_vpids, nav_targets, traj, gmaps, t):
        ...
        for i, ob in enumerate(obs):
            #TODO: mask stopeped and unwanted step
    ...
    def train(
        ...
        if args.only_cot_training:
            ...
            if args.stage == 'pretrain' or step % 2 == 0:
                ...
                #
                #     loss += loss2
            # print(f"mp3d_agent_only_cot.py is currently only for cot warmup with IL")
    ...
    def rollout(
        ...
        for t in range(max_action_len):
            ...
            with context():
                ...
                for i, ob in enumerate(obs):
                    for vp in nav_inputs['gmap_vpids'][i]:
                        # print("TODO")
                        TODO = True
                # TODO
                ...
                if enable_navigation_cot:
                    ...
                    if self.args.test_with_cot_gt and validate:
                        ...
                        if not validate:
                            ...
                            batch_generated_sentences_navigation_cot = []
                            for i in range(batch_size):
                                traj[i]['generated_sentences_navigation_cot'] = output["generated_sentences_navigation_cot"][i]
                                traj[i]['navigation_cot_gt'] = nav_inputs['navigation_cot_gt'][i]
