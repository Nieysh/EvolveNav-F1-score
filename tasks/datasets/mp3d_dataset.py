...
class MP3DDataset(BaseDataset):
    def __init__(
        ...
        # # newly added
        # if self.args.cot_imagination_gt:
        #     if self.source == "R2R":
        #         if self.split == "train":
        #             imagination_gt_data = load_json(str(args.data_dir / f"{self.source}_{self.split}_enc_LLM_task_GT_LLMlandmark.json"))
        #         elif self.split == "val_unseen":
        #             imagination_gt_data = load_json(str(args.data_dir / f"{self.source}_{self.split}_enc_LLM_task_GT.json"))
        #     else:
        #         raise NotImplementedError("Imagination GT is currently only available for R2R train and val_unseen")
        #     self.imagiantion_gt = {}
        #     for i in range(len(imagination_gt_data)):
        #         for k1, v1 in imagination_gt_data[i]['imagination_GT'].items():  # k1: instr_id, v1: {'0':"xxx",...}
        #             self.imagiantion_gt[f'{self.source.lower()}_{k1}'] = v1
        ...
        # newly added: caption
        self.caption_dir = str(args.data_dir / 'captions')
    ...
    def _load_data(self, config, data_dir):
        ...
        if self.source == "R2R":
            ...
            self.data['r2r'], self.gt_trajs = self.load_data(anno_file=anno_file, debug=self.debug, visualize=self.args.visualize)

            # if self.args.cot_imagination_gt:
            #     imagination_instr_ids = list(self.imagiantion_gt.keys())
            #     gt_instr_ids = list(self.gt_trajs.keys())
            #     assert imagination_instr_ids == gt_instr_ids, print(f"missing instr: {[x for x in gt_instr_ids if x not in imagination_instr_ids]}")
            #     print(f"all imagination instr_ids matches with gt traj instr_ids")
            ...
            self.data["r2r_aug"], _ = self.load_data(anno_file=anno_file, debug=self.debug, visualize=self.args.visualize)
    ...
    def get_obs(self, items, env, data_type=None):
        ...
        for i, (feature, state) in enumerate(env.getStates()):
            ...
            ob = {
                ...
                # 'captions': load_json(os.path.join(self.caption_dir,state.scanId,state.location.viewpointId,f"{state.scanId}_{state.location.viewpointId}.json")),
                'landmarks': load_json(os.path.join(self.t2t_landmark_dir, state.scanId, state.location.viewpointId,
                                                    f"{state.scanId}_{state.location.viewpointId}.json")),

            }

            # if self.args.cot_imagination_gt:
            #     if item['instr_id'] in self.imagiantion_gt:
            #         ob.update({
            #             'imagination_GT': self.imagiantion_gt[item['instr_id']], # if item['instr_id'] in self.imagiantion_gt else {},
            #         })
            ...
            if self.obj_feat_db is not None:
                ...
                # if False:  #
                if self.args.enable_RL_A2C and ob['instr_id'] in self.gt_trajs:
                    ob['distance'] = self.shortest_distances[ob['scan']][ob['viewpoint']][item['path'][-1]]
                    print(f"ob['distance']: {ob['distance']}")
                else:
                    ob['distance'] = 0
            obs.append(ob)
        return obs
