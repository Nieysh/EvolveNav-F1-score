    ...
    if args.mode=="test":
        ...
        for epoch in range(resume_from_epoch, args.num_epochs):
            ...
            if args.rank==0:
                ...
                #assert args.enable_navigation_cot == args.only_cot_training
                should_save_checkpoint = ((epoch+1)%5)==0
                history_ckpts = []
                if args.stage == 'multi' and should_save_checkpoint:
                    # if len(history_ckpts) > args.max_saved_checkpoints:
                    #     remove_epoch = history_ckpts[-1]
                    #     remove_model_path = Path(args.output_dir) / f"epoch_{remove_epoch}.pt"
                    #     if os.path.exists(remove_model_path):
                    #         os.remove(remove_model_path)
                    #         logger.info(f"Remove Checkpoint at Epoch {remove_epoch}...")
