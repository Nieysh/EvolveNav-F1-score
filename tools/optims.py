...
def check_checkpoint_critic(args, model, optimizer, lr_scheduler, logger) -> int:
    resume_from_epoch = 0
    if args.resume_from_critic_checkpoint is not None:
        if args.rank == 0:
            logger.info(f"Loading critic checkpoint from {args.resume_from_critic_checkpoint}")
        checkpoint = torch.load(args.resume_from_critic_checkpoint, map_location="cpu")
        model_state_dict = model.state_dict()
        state_disk = {k.replace('module.', ''): v for k, v in checkpoint['model_state_dict'].items()}
        update_model_state = {}
        for key, val in state_disk.items():
...
def dist_model_for_critic(args, model, logger):
    logger.info("*************** init critic model *************** ")
    # args.rank: global rank.
    total_gpus = torch.cuda.device_count()
    device_id = args.rank % total_gpus
    ...
    resume_from_epoch = check_checkpoint_critic(
        args, model, optimizer, lr_scheduler, logger,
    )

    param_sums = sum(p.numel() for p in model.parameters() if p.requires_grad)
    logger.info("critic model initialized with {:.2f} M trainable parameters".format(param_sums / 1000 ** 2))
    if args.distributed:
        from torch.nn.parallel import DistributedDataParallel as DDP
        model = DDP(model, device_ids=[device_id], find_unused_parameters=True)
        # model._set_static_graph()
